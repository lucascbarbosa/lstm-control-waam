...
class MPC:
    def __init__(self, bead_idx):
        ...
        self.u_min = self.process_input_train.min(axis=0)[0]
        self.u_max = self.process_input_train.max(axis=0)[0]
    ...
    def create_control_vector(self, i):
        ...
        for i in range(control_vector.shape[0]):
            control_vector[i] = np.dot(
                self.model_ss.C,
                np.dot(
                    np.linalg.matrix_power(self.model_ss.A, i+1),
                    self.model_ss.B
                )
            )
    ...
    def compute_step(self):
        ...
        for i in range(self.N):
            ...
            for j in range(self.process_outputs):
                y_row = self.predict_output(u_row)
    ...
    def predict_output(self, u):
        u = np.sqrt(u)
        self.x_row = np.dot(self.model_ss.A, self.x_row) +\
            np.dot(self.model_ss.B, u)
        y = list(np.dot(self.model_ss.C, self.x_row) +
                 np.dot(self.model_ss.D, u))[0]
        return y
